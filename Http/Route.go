...
func GetDnsData1(w http.ResponseWriter, r *http.Request) {
...
type RespCeyeData struct {
	Meta    RespCeyeDataMeta `json:"meta"`
	Dnslogs []Dnslogs        `json:"data"`
}
type RespCeyeDataMeta struct {
	Code    int    `json:"code"`
...
func Getceye(w http.ResponseWriter, r *http.Request) {
	key := r.FormValue("token")
	if Core.VerifyToken(key) {

		var dnslogs []Dnslogs

		//dnsAll := Dns.D.Get(key)

		dns := Dns.D.GetAllD(key)
		...
		for _, d := range dns {
			...
			a := Dnslogs{
				Id:         strconv.FormatInt(d.Time, 10),
				Name:       d.Subdomain,
				RemoteAddr: d.Ipaddress,
				CreatedAt:  time,
			}
			dnslogs = append(dnslogs, a)
		}

		json := RespCeyeData{
			Meta: RespCeyeDataMeta{
				Code:    200,
				Message: "OK",
			},
			Dnslogs: dnslogs,
		...
		fmt.Fprint(w, JsonRespData(json))
